 ...
 *
 */
public class VoskKaldiRunnable implements Runnable {
    private Logger logger = LoggerFactory.getLogger(VoskKaldiRunnable.class);
     ...
     */
    private final KaldiRecognizer speechRecognizer;
     ...
     */
    public VoskKaldiRunnable(KaldiRecognizer speechRecognizer, AudioStream audioStream) {
    ...
    public void run() {
        try {
            ...
            while ( !this.isAborting ) {
                ...
                if (size > 0) {
                    isRecValid = this.speechRecognizer.AcceptWaveform(buffer);
                    result = getResult(isRecValid);
                }
                else {
                    result = "";
                }
    ...
    private String getResult(boolean isRecValid) {
        String result;
        // Process result of previous stream read
        if (isRecValid) {

            result = this.speechRecognizer.Result();
            logger.info("Command recognized: {}", result);
            // The API doesn't return confidence
            //float confidence = 1;
            //sttListener.sttEventReceived(new SpeechRecognitionEvent(resutl, confidence));
        }
        else {

            result = this.speechRecognizer.PartialResult();
